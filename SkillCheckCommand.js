 ...
 *
 * PAIR WARNING: Keep "SkillUnlockNPC.js" in a SEPARATE tab — nearly identical helpers/events; combining overwrites handlers.
 */
...
/*
 * The Meditation script stores the shared trial in this world.
 */
 ...
 * MEDITATION TRIALS
 *
 * This list must match the list in your Meditation script.
 * ============================================================
 */
...
function showLockedSagaSkill(
    ...
    var level = getSkillLevelSafe(
        skills,
        skillId
    );

    var max = getSkillMaxSafe(
        skills,
        skillId,
        fallbackMax
    );
...
function showFlight(
    ...
    /*
     * Clean up old millisecond values.
     */
    if (
        required > 0 &&
        progress >
...
function showMeditation(
    ...
    if (level < 1) {
        ...
        showCurrentMeditationTrial(player);
...
function trigger(event) {
    ...
    try {
        ...
        var dmzLevel =
            Number(data.getLevel());

        var kiDamage =
            Number(data.getKiDamage());

        var maxEnergy =
            Number(data.getMaxEnergy());
