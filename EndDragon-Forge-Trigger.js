...
var END_DRAGON_TRIGGER_SPAWN = 50;
var END_DRAGON_TRIGGER_CLEANUP = 51;

function init(event) {
    try {
        print("[EndDragonForge] Trigger bridge v1.1.0 loaded (forwards triggers " +
            END_DRAGON_TRIGGER_SPAWN + "/" + END_DRAGON_TRIGGER_CLEANUP +
            " to player scripts).");
    } catch (e) {}
}
...
function trigger(event) {
    try {
        ...
        if (id !== END_DRAGON_TRIGGER_SPAWN && id !== END_DRAGON_TRIGGER_CLEANUP) return;

        /* Real player source → Global Player scripts already handled it. */
        if (isRealPlayerEntity(event.entity)) {
            try {
                print("[EndDragonForge] trigger " + id + " already handled via player source " +
        ...
        if (player == null) {
            try {
                print("[EndDragonForge] trigger " + id + ": no online player for '" +
                    playerName + "'. Usage: noppes script trigger " + id + " <PlayerName>");
            } catch (e1) {}
            return;
        }
        ...
        if (mc == null) {
            try { print("[EndDragonForge] trigger " + id + ": missing MC entity for " + playerName); } catch (e3) {}
            return;
        }
        ...
        if (data == null || data.scriptData == null) {
            try { print("[EndDragonForge] trigger " + id + ": player has no scriptData"); } catch (e9) {}
            return;
        }

        try {
            print("[EndDragonForge] forwarding trigger " + id + " to player scripts for " +
                str(player.getName()));
        } catch (e10) {}
