...
============================================================
 ...
 INSTALL (CustomNPCs) — FORGE SCRIPTS ONLY:
   CustomNPCs → Global → Forge → ONE dedicated tab with ONLY this file → Enable
   Do NOT combine with other Forge scripts in the same tab (init collisions).
   Do NOT put this in Global Player tabs.
 ...
 */
var NpcAPI = null;
try { NpcAPI = Java.type("noppes.npcs.api.NpcAPI"); } catch (eNpc) { NpcAPI = null; }
...
function playerInteractEventRightClickBlock(event) {
    try {
        ...
        var entity = event.entity;
        if (entity == null) {
            try { if (NpcAPI != null) entity = NpcAPI.Instance().getIEntity(forgeEvent.getEntity()); } catch (e0) {}
        }
        if (!isPlayerEntity(entity)) return;
        if (hasBypass(entity)) return;
    ...
    try {
        ...
        try {
            if (player == null && forgeEvent.player != null) {
                if (NpcAPI != null) player = NpcAPI.Instance().getIEntity(forgeEvent.player);
            }
        } catch (e2) {}
...
/* No Global Player tick() — this script is Forge-only to avoid overwriting
 * other player tabs' tick handlers if someone pastes it in the wrong place. */
